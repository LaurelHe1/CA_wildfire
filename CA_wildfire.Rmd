...
fire_2020 %>% ggplot(aes(x=duration_hrs, y=incident_acres_burned)) + geom_point() + scale_x_log10() + scale_y_log10() + xlab("log Duration (hours)") + ylab("log(Total Acres Burned)")
```
```{r}
fire_2021 = new_fire %>% filter(year(start_DT) == 2021)
fire_2021 %>% ggplot(aes(x=duration_hrs, y=incident_acres_burned)) + geom_point() + scale_x_log10() + scale_y_log10() + xlab("log Duration (hours)") + ylab("log(Total Acres Burned)")
```

```{r}
# The lack of linear relationship makes our model pretty unreliable
model = lm(incident_acres_burned ~ duration_hrs, data = fire_2020) 
library(car)
bc = boxCox(model)
lambda = bc$x[which(bc$y == max(bc$y))] #Extracting the best lambda value.
fire.bc = (fire_2020$incident_acres_burned^lambda - 1)/lambda #Applying the Box-Cox transformation.

model.bc = lm(fire.bc ~ fire_2020$duration_hrs) #Creating a new regression based on the transformed variable.
...
```
```{r}
model2 = lm(incident_acres_burned ~ duration_hrs, data = fire_2021) 
library(car)
bc2 = boxCox(model2)
lambda2 = bc2$x[which(bc2$y == max(bc2$y))] #Extracting the best lambda value.
fire.bc2 = (fire_2021$incident_acres_burned^lambda2 - 1)/lambda2 #Applying the Box-Cox transformation.

model.bc2 = lm(fire.bc2 ~ fire_2021$duration_hrs) #Creating a new regression based on the transformed variable.
summary(model.bc2) #Assessing the output of the new model.
```
```{r}
plot(model.bc2)
```
...
```
# Model prediction has high true positives, which is good, but the false predictions or error rate is still really high (12+29)/152 = 27.0%. Our accuracy is (39+72)/152 = 73.0%. Yet, if we were to simply predict "major incident" for every fire uniformly, we would have an accuracy of 121/190 = 63.7%. That's why the overall test of deviance was insignificant -- predicting only the intercept of the baseline probability was just as sufficient.
```{r}
